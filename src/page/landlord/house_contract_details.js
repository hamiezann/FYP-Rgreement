...
const nodeURL = process.env.REACT_APP_BLOCKCHAIN_NODE;
...
const HouseContractDetails = () => {
  ...
  useEffect(() => {
    const fetchContractDetails = async () => {
      try {
        const provider = new ethers.JsonRpcProvider(nodeURL.replace(/^https:\/\//, 'http://'));
        ...
        const depositAmountEther = ethers.formatEther(depositAmountWei);
        setDepositAmount(depositAmountEther);

        const remainingdepositAmountWei = contractDetails.remainingDeposit;
        const remainingdepositAmountEther = ethers.formatEther(remainingdepositAmountWei);
